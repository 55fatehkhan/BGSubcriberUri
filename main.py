...
def get_bpp_public_key_from_header(auth_header):
    ...
    if status_code == 200:
        print("SubscriberUrl of BG:    ", response[0]['subscriber_url'])
...
@app.route('/bgSubscriberUri', methods=['POST'])
def verify_authorisation_header(auth_header=AuthHeader, request_body_str=request_body_raw_text,
    ...
    if created <= current_timestamp <= expires:
        ...
        bg_subscriberUrl = get_bpp_public_key_from_header(auth_header)  # changes
        print("BG Subscriber Url  ", bg_subscriberUrl)


        return jsonpickle.encode(bg_subscriberUrl)
